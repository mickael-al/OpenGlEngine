...
void main()
{
    ...
    if (threshold >= 0.0f)
    {
        vec3 bright = vec3(0.2126, 0.7152, 0.0722);
        if (dot(a, bright) <= threshold) { a = vec3(0.0f); }
        if (dot(b, bright) <= threshold) { b = vec3(0.0f); }
        if (dot(c, bright) <= threshold) { c = vec3(0.0f); }
        if (dot(d, bright) <= threshold) { d = vec3(0.0f); }
        if (dot(e, bright) <= threshold) { e = vec3(0.0f); }
        if (dot(f, bright) <= threshold) { f = vec3(0.0f); }
        if (dot(g, bright) <= threshold) { g = vec3(0.0f); }
        if (dot(h, bright) <= threshold) { h = vec3(0.0f); }
        if (dot(i, bright) <= threshold) { i = vec3(0.0f); }
        if (dot(j, bright) <= threshold) { j = vec3(0.0f); }
        if (dot(k, bright) <= threshold) { k = vec3(0.0f); }
        if (dot(l, bright) <= threshold) { l = vec3(0.0f); }
        if (dot(m, bright) <= threshold) { m = vec3(0.0f); }
    }
