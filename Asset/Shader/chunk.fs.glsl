...
const PbrData getMatrix(uvec3 p, vec3 worldPos, vec3 blend)
{
    PbrData pd;
    Biome b = biomeData[p.x + p.y * 5 + p.z * 25];    
...
PbrData trilinearMatrixSample(vec3 buvw,vec3 WorldPos,vec3 normal)
{
    vec3 gridPos = buvw * float(MATRIX_SIZE - 1);
    ...
    uvec3 positions[8] = uvec3[](
        uvec3(p0.x, p0.y, p0.z),
        uvec3(p1.x, p0.y, p0.z),
        uvec3(p0.x, p1.y, p0.z),
        uvec3(p1.x, p1.y, p0.z),
        uvec3(p0.x, p0.y, p1.z),
        uvec3(p1.x, p0.y, p1.z),
        uvec3(p0.x, p1.y, p1.z),
        uvec3(p1.x, p1.y, p1.z)
        );

    PbrData result;
    result.Color = vec4(0.0);
    result.Normal = vec4(0.0);
    result.gOther = vec3(0.0);
    vec3 blend = getTriplanarBlend(normal);
    for (int i = 0; i < 8; ++i) 
    {
        if (w[i] < INTERPOLATION_EPSILON)
        {
            result.Color += vec4(w[i]);            
            continue; // skip tiny weights
        }

        PbrData pd = getMatrix(positions[i], WorldPos, blend);
        result.Color += pd.Color * w[i];
        result.Normal += pd.Normal * w[i];
        result.gOther += pd.gOther * w[i];
    }
...
{	
    ...
    PbrData pd = trilinearMatrixSample(uvw, WorldPos*0.25, Normal.xyz);
