...
layout(std430, binding = 4) buffer FieldValue
{
	float data[];
};

layout(std140, binding = 5) buffer NoiseParams
{
	uint width;
...
{   
	...
	uint ix = uint(mod(fragTexCoord.x, 1.0) * float(width));
	uint iy = uint(mod(fragTexCoord.y, 1.0) * float(height));

	// Calcul de l'index dans le buffer 1D
	uint index = ix + iy * width;

	float v = data[index];

	gColor = ubm.ubm[imaterial].albedo * col * vec4(Color, 1.0) * v;//(v > sin(ubd.u_time) ? 1.0 : 0.0);
